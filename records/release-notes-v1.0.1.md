...
Windows 本地修复版 ChatGPT 桌面客户端 —— 官方包隔离运行，无内核内存泄漏。**单 exe 一键安装**。
...
| **单 exe 一键安装** | 只需下载 `ChatGPT-Fix-Setup.exe`（Rust 原生 GUI，~2MB）：双击 → 自动探测官方包 → 复制到用户目录 → 写指针 → 创建快捷方式 → 注入 Token 插件 → 启动，全程无黑框、启动快 |
...
| 文件 | SHA-256 |
|---|---|
| ChatGPT-Fix-Setup.exe | `（构建后更新）` |

## 安装使用

1. 下载 `ChatGPT-Fix-Setup.exe`（单文件，~2MB）
2. 双击运行（默认管理员提权，自动请求 UAC）
...
- Rust `x86_64-pc-windows-msvc`，纯标准库（win32 最小 FFI），无外部 crate 依赖（Launcher/Manager/Packer/Setup）
- 230+ 项测试通过，clippy 零警告
- Setup 为 Rust 原生 Win32 GUI（纯 FFI，无外部依赖），从初版 .NET WinForms 安装器演进
